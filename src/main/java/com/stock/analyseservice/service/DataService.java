...
import com.stock.analyseservice.util.Util;
...
public class DataService {
    ...
    @Autowired
    private StockRealtimeRepository realtimeRepository;
...
//                    lineData.put("code", Float.valueOf(todayData.getStockCode()));
    ...
    private void writeResultData(List<Map<String, Float>> trainData, File trainFile) throws IOException {
        ...
        try {
            ...
            log.error("thread sleep error:{}", ignored.getMessage());
        }
    }
    ...
    public void initRealTimeData(Date start, Date end, List<String> codes, File trainData, int timeRange) throws IOException {
        //todo 分批次写入
        List<StockRealtime> realtimeList = realtimeRepository.findByTimeBetweenAndStockCodeIn(start, end, codes);
        Map<String, Map<String, Float>> codeWithTimeAndDataMap = new LinkedHashMap<>();
        for (StockRealtime realtime : realtimeList) {
            Map<String, Object> map = Util.beanToMap(realtime);
            if (map == null) {
                continue;
            }
            ...
            if (illegal) {
                Long time = realtime.getTime().getTime() / 60000 * 60000;
                String mapKey = realtime.getStockCode() + "_" + time;
                codeWithTimeAndDataMap.put(mapKey, data);
            }
        }
        parseRealtimeClassify(codeWithTimeAndDataMap, timeRange);
        writeResultData(new ArrayList<>(codeWithTimeAndDataMap.values()), trainData);
    }

    private void parseRealtimeClassify(Map<String, Map<String, Float>> codeWithTimeAndDataMap, int timeRange) {
        for (Map.Entry<String, Map<String, Float>> dataMap : codeWithTimeAndDataMap.entrySet()) {
            String[] codeWithTime = dataMap.getKey().split("_");
            if (codeWithTime.length > 1) {
                Map<String, Float> thisData = dataMap.getValue();
                Float nowPrice = thisData.get("now");
                Long time = Long.parseLong(codeWithTime[1]);
                String code = codeWithTime[0];
                Boolean up_1 = false;
                Boolean up_2 = false;
                Boolean up_3 = false;
                Boolean up_4 = false;
                ...
                for (int i = 1; i <= timeRange; i++) {
                    Long nextTime = time + 60000 * i;
                    String nextKey = code + "_" + nextTime;
                    if (codeWithTimeAndDataMap.containsKey(nextKey)) {
                        Map<String, Float> nextData = codeWithTimeAndDataMap.get(nextKey);
                        Float nextPrice = nextData.get("now");
                        Float rate = (nextPrice - nowPrice) / nowPrice;
                        if (rate > 0.05) {
                            up_1 = true;
                            up_2 = true;
                            up_3 = true;
                ...
                thisData.put("up_1", up_1 ? 1f : 0f);
                thisData.put("up_2", up_2 ? 1f : 0f);
                thisData.put("up_3", up_3 ? 1f : 0f);
                thisData.put("up_4", up_4 ? 1f : 0f);
                thisData.put("up_5", up_5 ? 1f : 0f);
            }
        }
    }
